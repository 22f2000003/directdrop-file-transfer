# DirectDrop Deployment Guide

This guide outlines several easy deployment options for the DirectDrop application, covering both the Java backend and Next.js frontend.
...
1. Create a Heroku app and deploy:
   ```bash
   heroku create directdrop-backend
...
5. Use a process manager like PM2:
   ```bash
   # For the backend
   pm2 start --name directdrop-backend java -- -jar target/p2p-1.0-SNAPSHOT.jar
...
6. Set up Nginx as a reverse proxy using the provided `nginx.conf.example` as a template:
   ```bash
   sudo cp nginx.conf.example /etc/nginx/sites-available/directdrop
   sudo ln -sf /etc/nginx/sites-available/directdrop /etc/nginx/sites-enabled/
   sudo nginx -t
...
Since DirectDrop is a P2P application that uses dynamic ports for file sharing:
