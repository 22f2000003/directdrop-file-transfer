...
# DirectDrop VPS Setup Script
# This script helps set up DirectDrop on a fresh Ubuntu/Debian VPS

# Exit on error
set -e

echo "=== DirectDrop VPS Setup Script ==="
echo "This script will install Java, Node.js, Nginx, and set up DirectDrop."
...
# git clone https://github.com/yourusername/directdrop.git
# cd directdrop
...
# Create the directdrop configuration file with the correct content
echo "Creating /etc/nginx/sites-available/directdrop..."
cat <<EOF | sudo tee /etc/nginx/sites-available/directdrop
server {
    listen 80;
...
# Create the symbolic link to enable the directdrop site
sudo ln -sf /etc/nginx/sites-available/directdrop /etc/nginx/sites-enabled/directdrop
...
if [ $? -eq 0 ]; then
    ...
    echo "Nginx configuration test failed. Please check /etc/nginx/nginx.conf and /etc/nginx/sites-available/directdrop."
    exit 1
fi
...
pm2 start --name directdrop-backend java -- -cp "$CLASSPATH" p2p.App

# Start frontend with PM2
echo "Starting frontend with PM2..."
cd ui
pm2 start npm --name directdrop-frontend -- start
cd ..
...
echo "=== Setup Complete ==="
echo "DirectDrop is now running on your VPS!"
